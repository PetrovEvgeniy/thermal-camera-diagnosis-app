...
class FlirImageExtractor:
    ...
    def process_image(self, flir_img_filename):
       ...
       # self.rgb_image_np = self.extract_embedded_image()
        ...
        # Apply mask based on the image name

        if "Kettle" in self.flir_img_filename:
            # Do not apply mask for Kettle images
            pass # Do nothing
        else:
            self.apply_mask()
    ...
    def apply_mask(self):
        """
        Applies the corresponding mask based on the image name (CAM1 or CAM2).
        """
        
        # Get the SNAP environment variable to form the mask file path
        mask_dir = os.environ["SNAP"] + "/config/python-code/masks"
        
        # Determine the mask based on the image name
        if "CAM1" in self.flir_img_filename:
            mask_filename = os.path.join(mask_dir, 'MASK_CAM1_thermal.png')
        elif "CAM2" in self.flir_img_filename:
            mask_filename = os.path.join(mask_dir, 'MASK_CAM2_thermal.png')
        else:
            raise ValueError("No valid mask found for the image name. Ensure CAM1 or CAM2 is part of the filename.")

        # Load the mask and apply it to the thermal image
        mask = cv2.imread(mask_filename, cv2.IMREAD_GRAYSCALE)
        if mask is None:
            raise ValueError("Mask file not found or invalid. Ensure the mask is located correctly.")

        if self.is_debug:
            print(f"Applying mask: {mask_filename}")

        # Ensure the mask and thermal image are of the same size
        if self.thermal_image_np.shape != mask.shape:
            raise ValueError("Mask and thermal image dimensions do not match.")

        # Apply the mask
        self.thermal_image_np = np.multiply(self.thermal_image_np, mask / 255.0)
    ...
    def get_thermal_np_normalized(self):
        """
        Return the last extracted thermal image in a normalized PIL image format
        :return:
        """
        ...
        thermal_normalized = thermal_np 
        img_thermal = np.uint8(cm.inferno(thermal_normalized) * 255)
    ...
    def get_thermal_image(self):
        ...
        thermal_np = self.thermal_image_np
        min_val = self.min_temp_range
        max_val = self.max_temp_range
        
        thermal_normalized = (thermal_np - min_val) / (max_val -  min_val)
        img_thermal = Image.fromarray(np.uint8(cm.inferno(thermal_normalized) * 255))

        return img_thermal

    def get_thermal_image_cv(self):
        """
        Return the last extracted thermal image in a normalized OpenCV format 
        :return:
        """
        thermal_np = self.thermal_image_np
        min_val = self.min_temp_range
        ...
        img_cv = cv2.cvtColor(img_thermal, cv2.COLOR_RGB2BGR)  # Convert RGB to BGR for OpenCV
